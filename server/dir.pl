...
#
# $Id: dir.pl,v 2.21 1994/11/08 23:57:57 sanders Exp $
#
...
sub retrieve {
    local($path, $filter) = @_;
    $path = &virtual'resolve_twiddle($path) if defined &virtual'resolve_twiddle;
...
sub wrap_file {
    ...
    &seize($FD, &LOCK_SH) if $plexus{'strict_lock'} eq 'true';
...
sub index_dir {
    ...
    if ($plexus{'picky_dirs'} eq 'true' && !$trailing_slash) {
	if ($plexus{'redirects_ok'} eq 'true') {
	    &main'redirect("$path/", $query, 'moved');
	} else {
	    &main'error('bad_request', "Directory requests must end with /");
	}
    }

    local($ndx);
    foreach $ndx (split(/ /, $plexus{'index'})) {
	$ndx = "$dir/$ndx";
	do { &wrap_file($ndx); return; } if -f $ndx;
    }

    $plexus{'autoindex'} eq 'true' || &error('bad_request', "$dir is a directory");
    ...
    eval '/$filter/';			# test filter regexp
    &error('bad_request', $@) if $@;
...
EOT
	...
	$_ .= "/" if &S_ISDIR($mode);
	$file = &main'printable($_);
        print "<A NAME=$count HREF=\"/$dir/$file\">$_</A>\n";
	$count++;
    }
    closedir(DH);
...
sub add_translations {
	...
	@conversions = split(/:/, $map);
		...
		# get rid of Content-length because we no longer know it
		undef $main'mime_headers{'Content-length'};
	        last CONV;
	    }
	}
    }
